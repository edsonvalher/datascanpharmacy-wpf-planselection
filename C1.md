...
graph TD
    ...
    System -->|processes payment transactions| PaymentNetwork
    System -->|sends financial transactions| QuickBooks
...
| **Fax Service** | Sends and receives prescriptions and clinical documents to and from prescribers. | `FaxClient/FaxAPI_EtherFax.cs`, `RingCentralClient` in `Winpharm-main/src/FaxClient/` |
| **Shipping Carriers** | Receives delivery orders from the pharmacy and returns tracking information. | `ShippingAPI/` in `Winpharm-main/src/`, `ShippingLib/` in `POS-main/` |
...
| **Payment Network** | Processes credit card and payment transactions at point of sale. Returns approval or denial with transaction reference. | `POSLib/PaymentTypes/CreditCardAPI_CardPointe.cs`, `CreditCardAPI_FirstData.cs`, `CreditCardAPI_NETePay.cs` in `POS-main/` |
| **QuickBooks** | Receives end of day financial transactions and invoice data from POS. | `POSLib/QuickBooksLib.cs`, `Lib/Interop.QBXMLRP2.dll` in `POS-main/` |
